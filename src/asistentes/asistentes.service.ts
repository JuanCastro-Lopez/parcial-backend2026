...
export class AsistentesService {
  ...
  async create(createAsistenteDto: CreateAsistenteDto) {
    ...
    const existe = await this.asistenteRepository.findOne({
      where: { codigoTicket: createAsistenteDto.codigoTicket },
    });

    if (existe) {
      throw new ConflictException(
        `El codigoTicket "${createAsistenteDto.codigoTicket}" ya existe`,
      );
    }
  ...
  async update(id: number, updateAsistenteDto: UpdateAsistenteDto) {
    ...
    if (
      asistenteData.codigoTicket &&
      asistenteData.codigoTicket !== asistente.codigoTicket
    ) {
      const existe = await this.asistenteRepository.findOne({
        where: { codigoTicket: asistenteData.codigoTicket },
      });

      if (existe) {
        throw new ConflictException(
          `El codigoTicket "${asistenteData.codigoTicket}" ya existe`,
        );
      }
    }
